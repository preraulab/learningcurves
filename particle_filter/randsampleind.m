function [y,  ind]= randsampleind(n, k, varargin)
%RANDSAMPLEIND Random sample, with or without replacement, RETURNS INDICES.
...
%                 error(message('stats:randsample:EmptyPopulation'));
        ...
        elseif isempty(w)
            ...
            if defaultStream
                [~, ind] = histc(rand(k,1),edges);
            else
                [~, ind] = histc(rand(s,k,1),edges);
            end
        end
        
    % Sample without replacement
    case 0%{false, 'false', 0}
        if k > n
