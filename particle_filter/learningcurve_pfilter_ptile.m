%LEARNINGCURVE_PFILTER_PTILE  Mixed binary/continuous learning-curve particle filter storing only percentiles
%
%   Usage:
%       [param_ests, particles] = learningcurve_pfilter_ptile()
%       [param_ests, particles] = learningcurve_pfilter_ptile(times, data)
%       [param_ests, particles] = learningcurve_pfilter_ptile(times, data, num_particles, smoother, prog_bar, plot_on)
%
%   Inputs:
%       times         : 1xT double - observation times in seconds -- required
%       data          : 2xT double - row 1 binary, row 2 continuous observations
%                       (use NaN for a missing row or for individual missing samples) -- required
...
%
%   Outputs:
%       param_ests : struct - estimated parameters and posterior summaries
%       particles  : struct - percentile summaries (not full particles)
%
%   Notes:
...
%           Cont  : z_t = alpha + beta*x_t + Ez, Ez ~ N(0, sig2z)
...
%       Z  = 1 + 0.3.*x + randn(1,length(x))*.5;
%       learningcurve_pfilter_ptile(times, [N; Z]);
%
%   See also: learningcurve_pfilter, binary_learningcurve_pfilter_ptile, weighted_random_index
%
